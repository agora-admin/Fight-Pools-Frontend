        ...
        <div className='sm:min-w-[250px] max-w-[300px] bg-card p-4 rounded-xl flex flex-col gap-2 justify-between w-full mt-6 sm:mt-0 sm:w-max'>
            ...
            <div className="flex items-center gap-2">
                ...
                <div className='flex flex-col'>
                    <h4 className='text-primary text-xs tracking-wide font-medium line-clamp-1'>{speaker1?.name}</h4>
                    <h4 className='text-primary text-xs tracking-wide font-medium line-clamp-1'>{speaker2?.name}</h4>
                </div>
            </div>

            {/* title */}
            <h3 className='text-primary text-sm my-2 font-gilroy font-semibold line-clamp-2'>{title ? title : "New Fight Pool"}</h3>
            {/* divider */}
            <div className='w-full mx-1 h-[0.5px] bg-[#CCEBFF] flex rounded-xl' />

            <div className="flex w-full items-center gap-2">
                <ChainTag chainId={activeChain?.id!} />
                <p className='text-secondary max-w-[20ch] text-[10px] font-Lexend'>Currently, only Polygon chain is supported.</p>
            </div>
        </div>
    );
}
