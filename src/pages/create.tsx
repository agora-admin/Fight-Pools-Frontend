...
const CreateDiscoursePage = () => {
    ...
    const [fightStyle,setFightStyle] = useState<string>('Select Fighting Style');
                    ...
                    <div className="flex flex-col w-full self-center md:justify-between px-6 sm:px-10 mx-10 lg:mx-0 gap-4">
                        <div className="w-full flex flex-col sm:flex-row sm:items-center justify-between py-10">
                            <div className="flex flex-col w-full sm:w-max  ">
                                <h4 className="text-primary font-gilroy">Start a new</h4>
                                <h2 className="text-orange font-gilroy font-bold text-4xl">Fight Pool</h2>
                                <p className="text-secondary text-[11px] max-w-[50ch] mt-2">
                                    Crowdfund fight pools with top fighters
                                </p>
                            </div>
                        ...
                        {/* Title input */}
                        {/* <label className="text-[14px] text-primary/60 font-Lexend" htmlFor="title">Topic For Discussion</label>
                        <TitleInput title={title} setTitle={setTitle} /> */}

                        {/* Speaker input */}
                        <label className="text-[14px] text-primary font-gilroy" htmlFor="speaker1">Invite Fighters to Match</label>
                        <SpeakerInput speakers={speakers} setSpeakers={setSpeakers} />

                        {/* Description (optional)*/}
                        <label className="text-[14px] text-primary font-gilroy" htmlFor="description">Description</label>
                        <div className="flex relative w-full">
                            <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={4} maxLength={256} className="max-w-full w-full input-s" id="description" placeholder="describe the purpose or initiative for the event" />
                            <p className="text-[10px] absolute bottom-2 right-2 text-[#c6c6c6]">{description.length} <span className="text-[#8e8e8e]"> / 256</span> </p>
                        </div>

                        {/* DropDown Menu */}
                        <label className="text-[14px] text-primary font-gilroy" htmlFor="description">Suggest Fight Rules</label>
                        <DropDown fightStyle={fightStyle} setFightStyle={setFightStyle} />

                        {/* Topics */}
                        <label className="text-[14px] font-gilroy text-primary">Suggest at least 3 fight locations</label>
                        <TopicsInput topics={topics} addTopic={handleAddTopic} removeTopic={handleRemoveTopic} />

                        {/* Funding Period Input */}
                        <label className="text-[14px] font-gilroy text-primary">Funding Period</label>
                        <FundingInput fundingPeriod={fundingPeriod} setFundingPeriod={setFundingPeriod} />

                        {/* Charity Percent Input */}
                        <label className="text-[14px] font-gilroy text-primary">Charity Percentage</label>
                        ...
                        <button onClick={handleSubmit} className="button-s w-max font-gilroy text-sm px-4 py-3">
                            Continue & fund
                        </button>
