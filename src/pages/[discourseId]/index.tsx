...
const DiscoursePage = () => {
    ...
    return (
        <div className="w-full h-screen overflow-x-clip">
            <Head>
                <title>Fight Pools by FIGHT SQUARE</title>
                    ...
                    <div className="flex flex-col md:flex-row md:justify-between mt-10 mx-10 lg:mx-0">
                        ...
                        {!loading && data && !error &&
                            <div className="flex flex-col gap-2 w-full md:flex-[0.6]">
                                <h3 className="text-primary font-semibold text-2xl">{data.getDiscourseById.title}</h3>
                                <div className="flex gap-2 items-center">
                                    ...
                                    <div className='flex items-center gap-2 text-sm font-semibold'>
                                        ...
                                        <p className='text-secondary text-xs'>{shortAddress(data.getDiscourseById.prop_starter)}</p>
                                    </div>
                                    <p className="text-secondary text-[10px] ">{getAgo(data?.getDiscourseById?.initTS)}</p>

                                </div>
                                <p className=" mt-4 w-full text-secondary text-sm leading-5 tracking-wide">{data.getDiscourseById.description}</p>
                                <div className="bg-card flex flex-col gap-2 p-6 rounded-xl w-full md:max-w-[80%] mt-2">
                                    <h4 className="text-sm text-primary">Timeline :</h4>
                                    <ul className="list-inside">
                                        {
                                            data.getDiscourseById.topics.map((item: string, index: number) => (
                                                <li className="text-secondary text-sm list-disc" key={index}>{item}</li>
                                            ))
                                        }
                                    </ul>
                                </div>
                                ...
                                <div className="flex items-center gap-4 mt-8">
                                    <div className="flex flex-col">
                                        <p className=" w-full text-primary text-sm leading-5 tracking-wide">Total Stake:</p>
                                        <h3 className="text-primary text-lg font-bold tracking-wider">{getFundTotal(data.getDiscourseById.funds)} {getCurrencyName(data.getDiscourseById.chainId)}</h3>
                                    </div>
                                ...
                                <>
                                    ...
                                    <div className="flex flex-col gap-2 bg-card rounded-xl p-4">
                                        ...
                                        <div className='flex items-center gap-1 mt-2'>
                                            ...
                                            <div className='flex flex-col'>
                                                <a href={`https://twitter.com/${data.getDiscourseById.speakers[0]?.username}`} className='hover:text-primary/60 text-primary/30 text-xs tracking-wide font-medium'>{data.getDiscourseById.speakers[0]?.name}</a>
                                                <a href={`https://twitter.com/${data.getDiscourseById.speakers[1]?.username}`} className='hover:text-primary/60 text-primary/30 text-xs tracking-wide font-medium'>{data.getDiscourseById.speakers[1]?.name}</a>
                                            </div>
                                        </div>
                                                    ...
                                                    .map((item: any, index: number) => (
                                                        <div key={index} className="flex flex-col gap-2 py-2">
                                                            <div className='flex items-center gap-2 text-[#616162] text-sm font-semibold'>
                                                                ...
                                                                <p className='text-secondary text-xs'>{shortAddress(item.address)}</p>
                                                            </div>
                                                            <div className="flex items-center justify-between">
                                                                <p className="text-secondary text-sm font-bold">{getFund(item.amount)} {getCurrencyName(data.getDiscourseById.chainId)}</p>
                                                                <p className="text-secondary text-[10px] ">{getAgoT(item.timestamp)}</p>
                                                            </div>
                                                        </div>
                                                    ))
                                            }
