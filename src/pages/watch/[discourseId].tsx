                            ...
                            <div className="flex gap-4 sm:flex-row flex-col w-full md:flex-[0.6]">
                                ...
                                <div className="flex gap-2 items-center">
                                    ...
                                    <div className='flex items-center gap-2 text-[#616162] text-sm font-semibold'>
                                        ...
                                        <p className='text-primary/60 text-xs'>{shortAddress(data.getDiscourseById.prop_starter)}</p>
                                    </div>
                                    <p className="text-primary/40 text-[10px] ">{getAgo(data?.getDiscourseById?.initTS)}</p>

                                </div>
                            </div>
                        }
                        { !sessionError && <h3 className="text-sm font-Lexend text-primary mt-6">Recordings <span className="text-secondary">/ sessions</span></h3>}
                        {
                            !error && sessionError &&
                            <div className="flex gap-4 flex-col w-full md:flex-[0.6] items-center pt-20">
                                <VideoSlash size='16' color="#526B7A" />
                                <p className="text-secondary text-xs font-Lexend">No recordings found</p>
                            </div>
                        }
                                ...
                                sessionData.getSessions.map((session: any) => (
                                    <div className="bg-card flex-col gap-4 p-4 rounded-lg" key={session.id}>
                                        <div className="flex items-center gap-2">
                                            <VideoOctagon size="16" color="#526B7A" />
                                            <p className="text-primary text-xs font-Lexend">{(session.id).slice(-20)}</p>
                                        </div>
                                        <div className="flex items-center gap-2 mt-2">
                                            <Calendar1 size="16" color="#526B7A" />
                                            <p className="text-secondary text-xs font-Lexend">{getTimeFromDate(new Date(session.createdAt))}</p>
                                            <p className="text-secondary/60 text-xs font-Lexend">{formatDate(new Date(session.createdAt))}</p>
                                        </div>
